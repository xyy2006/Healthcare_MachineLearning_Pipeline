...
#--------------------#
Survival_quantileReg <- function(data = data_input,response_variables = response_variables_vec, knowledge_driven_factors = knowledge_driven_factors_vec, data_driven_factors = data_driven_factors_vec, factors_annotation_table = factors_all, timeResponseVar = "LOS.x", eventResponseVar = "died",.seed=123,plotNameEssence = sprintf("Survival_glmnet_plot_at_%s",nowTimeToString()) ){
	# browser()
	...
	#-------#
	browser()
	result <- crq(formula_unionALLnY,data = data,na.action = na.omit, method = "Portnoy")	
	...
	#
	result <- cv.glmnet(x = new_X_modelExpanded_overall, y = Surv_obj, family="cox",alpha=0.5, lambda = NULL, nfolds=10, keep=T, penalty.factor = keepKnowledgeFactor_seq,parallel=T,standardize = F)	# 1 for lasso, 0 for ridge.
