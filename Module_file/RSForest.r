...
#--------------------#
RSForest <- function(data = data_input,response_variables = response_variables_vec, knowledge_driven_factors = knowledge_driven_factors_vec, data_driven_factors = data_driven_factors_vec, factors_annotation_table = factors_all, timeResponseVar = "LOS.x", eventResponseVar = "died",.seed=123,plotNameEssence = sprintf("RSForest_plot_at_%s",nowTimeToString()), add_varMarginalEffectPartialPlot = FALSE ){
	# browser()
	...
	data <- data[data[[timeResponseVar]] >=0,]	# cleaning time to include only those >=0. Under debate, whether to include 0 or not. Though here, include 0 will work for package randomForestSRC. 
	...
	#--------#
	characterFormula_survivalY <- sprintf("Surv( %s, %s)",timeResponseVar,eventResponseVar)
	formula_unionALLnY <- as.formula( sprintf("%s ~ %s + %s", characterFormula_survivalY ,paste(knowledge_driven_factors,collapse="+"), paste(data_driven_factors,collapse="+")  ) )
	...
	#-----plot in batch------#
	pdf(sprintf("%s_rfsrcSurvObjplot.pdf",plotNameEssence) )
	par(ps=12,cex=1)
	plot.rfsrc(result)
	dev.off()
	#
	pdf(sprintf("%s_rfsrcSurvObjplot_survival.pdf",plotNameEssence) )
	par(ps=12,cex=1)
	plot.survival(result)
	dev.off()
	...
	if (add_varMarginalEffectPartialPlot) {
		pdf(sprintf("%s_rfsrcSurvObjplot_varMarginalEffectPartialPlot.pdf",plotNameEssence) )
		par(ps=12,cex=1)
	...
	y_axis_var <- "mort"
	pdf(sprintf("%s_rfsrcSurvObjplot_varMarginalEffectPlot_%s.pdf",plotNameEssence,y_axis_var) )
	par(ps=12,cex=1)
	plot.variable(result,surv.type = y_axis_var)
	dev.off()
	#
	y_axis_var <- "rel.freq"
	pdf(sprintf("%s_rfsrcSurvObjplot_varMarginalEffectPlot_%s.pdf",plotNameEssence,y_axis_var) )
	...
	dev.off()
	#
	y_axis_var <- "surv"	# median follow up time survival.
	pdf(sprintf("%s_rfsrcSurvObjplot_varMarginalEffectPlot_%s.pdf",plotNameEssence,y_axis_var) )
	par(ps=12,cex=1)
	plot.variable(result,surv.type = y_axis_var)
	dev.off()
